...
test('a game folder Windows will not let us write to is refused in words, up front (#301)', () => {
  ...
  assert.equal((read('src', 'renderer', 'i18n.js').match(/errNoWriteAccess: '/g) || []).length, 3, 'in English, Arabic and Chinese');
});
...
test('the game sheet shows what the community found, before installing', () => {
  ...
  assert.equal((read('src', 'renderer', 'i18n.js').match(/sheetCommunityTitle: '/g) || []).length, 3, 'in English, Arabic and Chinese');
});
...
test('reactions and chat reach you as notifications', () => {
  ...
  for (const key of ['noticeReacted', 'noticeChatMention', 'noticeChatReply', 'noticeChatReaction']) {
    assert.equal((page.match(new RegExp(`${key}: `, 'g')) || []).length, 3, `${key} in English, Arabic and Chinese`);
  }
